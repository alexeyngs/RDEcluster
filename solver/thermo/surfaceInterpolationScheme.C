...
) const
{
    ...
    tmp<GeometricField<Type, fvsPatchField, surfaceMesh>> tsfs = interpolate(vf, weights(vf));
...
{
    tmp<GeometricField<Type, fvsPatchField, surfaceMesh>> tinterpVf = interpolate(tvf());
    tvf.clear();
    return tinterpVf;
}
