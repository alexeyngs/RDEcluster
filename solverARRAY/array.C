...
int main(int argc, char *argv[])
{
    ...
    for(const auto & file : std::filesystem::directory_iterator(thispath))
    {
        if(std::filesystem::is_directory(file.symlink_status())) // если iй файл - поддиректория
        {
            // Итак, знаем, что текущий файл - директория, но нужно проверить, является ли имя файла числом.
            std::string DirectoryName = file.path().string().substr(file.path().string().rfind("/") + 1, std::string::npos);
            // Если строку никак не преобразовать в этот тип, то возвращается 0.0
            double test = std::atof(DirectoryName.c_str());
            if(test != 0.0) directory_use(mesh, DirectoryName, FileName, file.path().string()); // Обработка папки
        }
    }
    ...
    out << "#     time    -    Xpeak   -   peak(atm)-   D    -    XCJ   -  c       -      U    -    molWeight   -   T     -      p(atm)" << std::endl;
    for(SCell & cell : DATA)
    {
        out.width(12); out << cell.Time;
        out.width(12); out << cell.Xpeak;
